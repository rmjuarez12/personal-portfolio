...
import { FaReact, FaPhp, FaNodeJs, FaWordpressSimple, FaShopify, FaElementor } from "react-icons/fa";
import { IoLogoJavascript } from "react-icons/io5";
import { TiHtml5 } from "react-icons/ti";
import { DiCss3 } from "react-icons/di";
import { SiMysql, SiSharp, SiExpress, SiTailwindcss } from "react-icons/si";
    ...
    <div className="mb-12">
      ...
      <div className="flex flex-row mb-6 flex-wrap">
        {languages.map((skill: SkillObj, index: number) => {
          return (
            <IconBox key={index} icon={skill.icon} label={skill.title} title={skill.title} className="size-24 m-3" iconClassName="text-4xl mb-2" />
          );
        })}
      </div>

      <h3 className="mb-5 text-3xl font-semibold">Technologies and Frameworks</h3>
      ...
      {generalSkills.map((skill: string, index: number) => {
        return (
          <div key={index} className="text-lime-500 border border-lime-500 m-1 py-1 px-2 inline-block font-semibold bg-lime-950">
            {skill}
          </div>
        );
      })}
